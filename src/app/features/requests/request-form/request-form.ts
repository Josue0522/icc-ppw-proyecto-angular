...
export class RequestForm {
  ...
  async submitRequest() {
    ...
    const user = await new Promise<any>((resolve) => {
      const sub = this.user$.subscribe((currentUser) => {
        resolve(currentUser);
        sub.unsubscribe();
      });
    });
    ...
    const programadoresResponse = await new Promise<any>((resolve) => {
      const sub = this.programadores$.subscribe((response) => {
        resolve(response);
        sub.unsubscribe();
      });
    });
    ...
    const programador = programadoresResponse.data.find(
      (p: any) => p.slug === this.programadorSlug
    );
